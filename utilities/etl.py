...
class PostgresqlDestination:
    ...
    def merge_tables(self, details):
        ...
        columns_info = schema_handle.get_column_info(table_name = table_name, schema_name = schema_name)
    ...
    def handle_schema_drift(self, df, details, columns_to_add, modified_cols):
        ...
        if modified_cols:
            for data in modified_cols:
                ...
                df.rename(columns = {data["column_name"]:f"{data['column_name']}_{str(data['data_type']).replace(' ', '_')}"}, inplace = True)
                return cur
