...
class DatabaseConnection:
    def __init__(self):
        self.postgres_hook = PostgresHook(postgres_conn_id = "themovies_con")
...
class PostgresqlDestination:
    ...
    def write_dataframe(self, df, details):
        ...
        schema_handle = SchemaDriftHandle()
        postgres_op = PostgresOperations()
        ...
        if not table_exists:
            ...
            response = df.to_sql(
                table_name,
                schema=schema_name,
                con=self.db_conn.engine,
                if_exists="append",
                index=False)
            print(f"Data was loaded successfully with {response} rows.")
    ...
    def merge_tables(self,df, details):
        ...
        print(f"{temp_response} rows loaded in {temp_table}")
